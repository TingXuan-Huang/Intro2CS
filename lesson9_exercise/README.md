# Lesson 9 exercise — a FRED client with caching, tidy, and storage
...
Open `fred_client.py` and implement the three functions documented in its
docstring:
...
- **`store_series(monthly, db_path, series_id)`** — append the tidy rows to a
  SQLite table, tagged with the series id, and return how many rows landed.
...
From **inside this folder** (`lesson9_exercise/`):
...
`python3 -m pytest test_lesson9.py -v` reports **all tests passing**. That means:
...
- `store_series` lands the right number of rows in `observations`, with `month`
  stored as text and `value` as real, and two series coexist in one database.

## Files

- `fred_client.py` — **you edit this** (the three functions).
- `test_lesson9.py` — the checker. Read it; it never reveals the implementations.
...
- `data/` — created at runtime for the cache and the SQLite db; gitignored.

## Where this leads

This client is not a throwaway: fetch-with-cache into SQLite is the data layer
of the integrated mini-project assignment (Lesson 11), where an LLM summarizes
what your pipeline stored — and of the capstone agent after it. Build it well
here and you reuse it there.
